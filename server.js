...
io.on('connection', function(socket) { 
  ...
  globalEmitter.on('tweet', function(object) {
    var formattedObject = format(object);
    var strippedObject = strip(formattedObject);
    var sentiment = analyseSentiment(strippedObject);
    formattedObject.colour = pickColour(sentiment);
    socket.emit('object', formattedObject)
  });
})
